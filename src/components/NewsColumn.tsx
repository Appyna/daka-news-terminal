...
const NewsColumn: React.FC<NewsColumnProps> = ({ column, onItemClick }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden">
      <div 
        className="p-2 font-extrabold text-sm uppercase tracking-wider border-b border-white/20 flex-shrink-0"
        style={{ color: '#F5C518' }}
      >
        {column.media}
      </div>
      <div className="flex-1 overflow-y-auto overflow-x-hidden w-full">
        {isLoading ? (
          ...
          <div className="space-y-0">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="p-2 border-b border-white/5 animate-pulse">
